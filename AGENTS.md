...
- **inspect then test after push for webhook/chat/form workflows**: Run `npx --yes n8nac test-plan <id>` first, then `npx --yes n8nac test <id>`. A Class A error is not a bug — tell the user. A Class B error is fixable — iterate.
...
```
Closes the dev cycle for webhook/chat/form workflows. Exits 0 on success or Class A (config gap — inform user). Exits 1 on Class B (wiring error — fix and re-test). Prefer `npx --yes n8nac test-plan` first when the payload is unclear.
